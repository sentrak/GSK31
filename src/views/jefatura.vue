  ...
  <div id="layout">
    ...
    <v-row justify="center">
      <v-dialog v-model="dialog" persistent max-width="600px">
        ...
        <!--FORMULARIO PARA CREAR EL JEJEFE -->
        <v-card color="#121D3C">
          <v-card-title id="cards_title">REGISTRAR JEFE</v-card-title>
          ...
          <v-card-text>
            <v-container>
              ...
              <!--FIN DATE PICKER -->
              <v-col class="camposGSK31">
                <p id="indices">Unidad:</p>
                <v-select :items="unidades" background-color="#eee" v-model="unidad" dense outlined></v-select>
              </v-col>
                   <v-col class="camposGSK31">
                <p id="indices">Rol:</p>
                <v-select :items="roles" background-color="#eee" v-model="rol" dense outlined></v-select>
              </v-col>
              <v-col class="camposGSK31">
...
import jefeCard from '../components/jefesCard'
export default {
  components: {
    jefeCard
  },
  data: () => ({
    ...
    imagen: '',
    contrasena: '',
    roles:["Jefe de grupo","Jefe de unidad","Sub jefe de unidad"],
    unidades: ['Familia', 'Manada', 'Tropa', 'Sociedad', 'Clan'],
    dialog: false,
    dialog2: false,
  ...
  methods: {
    validar_y_crear() {
      ...
      if (
        ...
        //hacemos la conexion y creamos el jefe
        this.axios
          .post('jefe/nuevo', this.validacion[0])
          .then(response => {
            this.dialog = false
            location.reload()
          })
          .catch(e => {
            alert(
              'Lo sentimos, ha ocurrido un error, verifique la información del jefe nuevamente'
            )
          })
      }
    }
